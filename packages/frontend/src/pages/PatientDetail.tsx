  ...
  return (
    <div>
      ...
      <div className="mt-2">
        ...
        {tab === 'examens' && <table className="data-table"><thead><tr><th>Date</th><th>Type</th><th>Résultat</th><th>Prescrit par</th></tr></thead><tbody>{hist?.examens?.map((e:any) => <tr key={e.id}><td>{new Date(e.date_examen).toLocaleDateString('fr-FR')}</td><td>{e.type_examen}</td><td>{e.resultat||'-'}</td><td>{`${e.medecin_prenom ?? ''} ${e.medecin_nom ?? ''}`.trim() || '-'}</td></tr>)}{!hist?.examens?.length && <tr><td colSpan={4} className="table-empty">Aucun</td></tr>}</tbody></table>}
