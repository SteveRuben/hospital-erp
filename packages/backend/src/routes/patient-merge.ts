...
router.get('/duplicates', authenticate, authorize('admin'), async (_req: AuthRequest, res: Response): Promise<void> => {
  try {
    const rows = await prisma.$queryRaw<Array<{
      id1: number; nom1: string; prenom1: string; dob1: Date | null; tel1: string | null;
      id2: number; nom2: string; prenom2: string; dob2: Date | null; tel2: string | null;
    }>>`
...
router.post('/merge', authenticate, authorize('admin'), validate(patientMergeSchema), async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    ...
    for (const table of tables) {
      try {
        // table name is from a hardcoded whitelist; safe to interpolate
        await prisma.$executeRawUnsafe(`UPDATE ${table} SET patient_id = $1 WHERE patient_id = $2`, keep_id, merge_id);
      } catch { /* table might not exist or no patient_id column */ }
    }

    // Archive the merged patient
    await prisma.patient.update({ where: { id: Number(merge_id) }, data: { archived: true } });
    ...
    await prisma.auditLog.create({
      data: {
        ...
        tableName: 'patients',
        recordId: keep_id,
        details: `Merged patient #${merge_id} into #${keep_id}`,
      },
    });
